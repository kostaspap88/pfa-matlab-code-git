...
% 6. Pushing the limit of PFA: Enhancced PFA on block ciphers, Xu et al.
...
% choose which cipher will be simulated
implementation_choice = 1;

% choose which attack will be performed on the cipher
attack_choice = 1;
    ...
    for j=1:no_attacks 
        ...
        switch implementation_choice
            ...
            case 3
                ...
                [plaintext_full, ciphertext_full] = implementation_fullAES(keyc.key_fullAES_correct, no_traces, sboxc);
        ...
        switch attack_choice
            ...
            case 1
                ...
                pfa_result.key_singlesbox{k,j} = attack_singlefault_singlesboxAES(ciphertext_singlesbox.singlefault, vsize, sboxc, faultc);
            ...
            case 2
                ...
                pfa_result.key_full{k,j} = attack_singlefault_fullAES(ciphertext_full.singlefault, vsize, sboxc, faultc); 
            ...
            case 3
                ...
                pfa_result.key_singlesbox_mle{k,j} = attack_singlefault_singlesboxAES_mle(ciphertext_singlesbox.singlefault, vsize, sboxc, faultc);
                % compare the result with keyc.key_byte_lastroundAES_correct
               
            case 4
                % MLE attack on the full AES implementation using the 2-step approach from 'PFA with few encryptions'
                pfa_result.key_fullAES_mle{k,j} = attack_singlefault_fullAES_mle(ciphertext_full.singlefault, vsize, sboxc, faultc);
                % compare the result with keyc.key_lastroundAES_correct

            case 5
                % MLE attack on the single-sbox round10 implementation when the fault is unknown from 'PFA in practice'
                pfa_result.key_singlesbox_unknownfault{k,j} = attack_singlefault_singlesboxAES_unknownfault(ciphertext_singlesbox.singlefault, vsize, sboxc, faultc);
                % compare the result with keyc.key_byte_lastroundAES_correct
               
            case 6
                % MLE attack on the full AES implementation when the fault is unknown from 'PFA in practice'
                pfa_result.key_fullAES_unknownfault{k,j} = attack_singlefault_fullAES_unknownfault(ciphertext_full.singlefault, vsize, sboxc, faultc);
                % compare the result with keyc.key_lastroundAES_correct
                
            case 7 
                % MLE attack on the single-sbox round10 implementation when both the fault and position are unknown from 'PFA in practice'
                pfa_result.key_singlesbox_unknownfaultandposition{k,j} = attack_singlefault_singlesboxAES_unknownfaultandposition(ciphertext_singlesbox.singlefault, vsize, sboxc, sboxinput_singlesbox(1) );
                % compare the result with keyc.key_byte_lastroundAES_correct
                
            case 8 
                % MLE attack on the full AES implementation when both the fault and position are unknown from 'PFA in practice'
                pfa_result.key_fullAES_unknownfaultandposition{k,j} = attack_singlefault_fullAES_unknownfaultandposition(ciphertext_full.singlefault, vsize, sboxc, plaintext_full(1,:) );
                % compare the result with keyc.key_lastroundAES_correct
                
            case 9
                % Statistical persistent fault analysis from 'SPFA: SFA on Multiple Persistent Faults' recovering a byte
                pfa_result.key_singlebyte_spfa{k,j} = attack_singlefault_fullAES_spfa(ciphertext_full.singlefault, vsize, sboxc, keyc.key_fullAES_correct);
            ...
            case 10
                ...
                pfa_result.key_singlesbox_duplication_nco{k,j} = attack_singlefault_singlesboxAES(ciphertext_singlesbox_duplication.nco, vsize, sboxc, faultc);
                pfa_result.key_singlesbox_duplication_rco{k,j} = attack_singlefault_singlesboxAES(ciphertext_singlesbox_duplication.rco, vsize, sboxc, faultc);
                pfa_result.key_singlesbox_duplication_vco{k,j} = attack_singlefault_singlesboxAES(ciphertext_singlesbox_duplication.zvo, vsize, sboxc, faultc);
                % compare the result with keyc.key_byte_lastroundAES_correct
                
            case 11
                % enhanced persistent fault analysis on the full AES
                pfa_result.key_fullAES_epfa{k,j} = attack_singlefault_fullAES_epfa(ciphertext_full.singlefault, vsize, sboxc, faultc);
                % compare the result with keyc.key_lastroundAES_correct
